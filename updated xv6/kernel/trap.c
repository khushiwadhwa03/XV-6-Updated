...
#include "proc.h"
#include "defs.h"
extern struct mlfq_queue mlfq[5];
...
void usertrap(void)
{
  ...
  else if (r_scause() == 13 || r_scause() == 15) // not 12
  {
    if (r_stval() == 0)
      setkilled(p);
  ...
  // give up the CPU if this is a timer interrupt. // alarm handling
  if (which_dev == 2)
  {
    p->tickstillnow++;
    if (p->alarmcheck == 0)
    {
      if (p->ticks >= 1)
      {
        if (p->tickstillnow >= p->ticks)
        {
          p->tickstillnow = 0;
          p->alarmcheck = 1;
          *(p->duptrap) = *(p->trapframe);
